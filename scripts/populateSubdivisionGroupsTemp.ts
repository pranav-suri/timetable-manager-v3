...
const data: SubdivisionGroup[] = [
  {
    groupName: "SY CS A",
    subdivisionNames: ["SY CS A1", "SY CS A2", "SY CS A3","SY CS A4"],
  },
  {
    groupName: "SY CS B",
    subdivisionNames: ["SY CS B1", "SY CS B2", "SY CS B3","SY CS B4"],
  },
  {
    groupName: "SY CS C",
    subdivisionNames: ["SY CS C1", "SY CS C2", "SY CS C3","SY CS C4"],
  },
  {
    groupName: "TY CS A",
    subdivisionNames: ["TY CS A1", "TY CS A2", "TY CS A3","TY CS A4"],
  },
  {
    groupName: "TY CS B",
    subdivisionNames: ["TY CS B1", "TY CS B2", "TY CS B3","TY CS B4"],
  },
  {
    groupName: "TY CS C",
    subdivisionNames: ["TY CS C1", "TY CS C2", "TY CS C3","TY CS C4"],
  },
    {
      groupName: "FLY CS A",
...
const timetables = await prisma.timetable.findMany({where: {name: "2025 EVEN"}});
...
console.log("===========")
  for (const group of data) {
    for (const subdiv of group.subdivisionNames) {
      const subdivId = subdivisionMap[subdiv];
      if (!subdivId) console.log(`\"${subdiv}\"`);
      // continue
